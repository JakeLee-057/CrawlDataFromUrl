...
def scrape_shopeefood_menu(url, sheet_name, worksheet_name):
    ...
    try:
        ...
        # 🔗 Ghi đường dẫn Google Sheet vào file (đúng worksheet)
        spreadsheet_url = sheet.spreadsheet.url
        worksheet_gid = sheet._properties['sheetId']
        full_url = f"{spreadsheet_url}/edit#gid={worksheet_gid}"
        with open("sheet_url.txt", "w", encoding="utf-8") as f:
            f.write(full_url)
        print(f"📎 Google Sheet URL: {full_url}")
